...
public class AdminLoginServiceImpl implements AdminLoginService {
    ...
    public Map<String, Object> getUserInfo(Map<String, Object> param) throws Exception {
        return adminMapper.getUserInfo(param);
    }
...
//        }
        ...
        session.setAttribute("login_admin_name", admin.get("admin_name"));	//세션에 사용자 정보 저장
        session.setAttribute("login_admin_email", admin.get("admin_email"));	//세션에 사용자 정보 저장
        
        // 로그인 성공 처리 (예: 세션 생성 또는 토큰 발급)
        result.put("login_admin", admin);
    ...
    public Map<String, Object> registerUser(Map<String, Object> param) throws Exception {
     ...
     // 비밀번호 암호화 (MD5 방식)
        String rawPassword = param.get("admin_password").toString();
        String hashedPassword = encryptMD5(rawPassword); // MD5 암호화 적용
        param.put("admin_password", hashedPassword);

        // 사용자 정보 저장
        try {
        	adminMapper.insertUser(param);
        } catch (Exception e) {
    ...
    public boolean isEmailDuplicated(Map<String, Object> param) {
        // Map에서 email 키 추출
        String admin_email = (String) param.get("admin_email");
    ...
    public boolean updateUserInfo(Map<String, Object> param, HttpSession session) throws Exception {
        int rowsAffected = adminMapper.updateUser(param);

        session.setAttribute("login_admin_name", param.get("admin_name"));
    ...
    public Map<String, Object> changePassword(Map<String, Object> param) throws Exception {
    	...
    	param.put("admin_password", encryptMD5(String.valueOf(param.get("admin_password"))));
    	Map<String, Object> isPasswordMatch = adminMapper.getPasswordCheck(param);
    	if(isPasswordMatch == null) {
            ...
            // 비밀번호 업데이트
		    int rowsAffected = adminMapper.updatePassword(param);
		    if (rowsAffected > 0) {
